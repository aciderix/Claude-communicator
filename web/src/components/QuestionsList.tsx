...
import { CornerDownRight, MessageSquareDashed, History } from 'lucide-react';
...
export default function QuestionsList({ questions, onAnswer }: QuestionsListProps) {
  ...
  const answeredQ = allQ.filter(q => q.status === 'answered').slice(0, 20);
    ...
    <div className="space-y-6 mb-8">
      ...
      {answeredQ.length > 0 && (
        <div className="space-y-3">
          <div className="flex items-center gap-2 text-slate-500 uppercase tracking-widest text-xs font-semibold px-1">
            <History className="w-4 h-4" /> Historique ({answeredQ.length})
          </div>
          <div className="space-y-2">
            {answeredQ.map(q => (
              <Card key={q.id} className="p-3 bg-slate-900/40 border border-white/5 space-y-1.5">
                <div className="flex items-center gap-2 text-[10px] text-slate-500 font-mono uppercase tracking-widest">
                  <span>{q.id} · de {q.from}</span>
                </div>
