...
function applyUserAction(u, actor, a = {}) {
  ...
  switch (a.action) {
    ...
    case 'post': {
      // message SPONTANÉ d'une session vers l'utilisateur (fil du dashboard)
      if (!a.body) throw new Error('Paramètre requis pour post : body.');
      const m = {
        id: `U${u.msgs.next_id++}`, ts: nowISO(), to: 'user', from: actor,
        body: String(a.body).slice(0, 8000),
        status: 'info', claimed_by: null, claimed_at: null, replies: [],
      };
      u.msgs.items.push(m);
      if (u.msgs.items.length > 200) u.msgs.items.splice(0, u.msgs.items.length - 200);
      return { changed: true, result: { type: 'posted', msg: m } };
    }
    case 'ask': {
